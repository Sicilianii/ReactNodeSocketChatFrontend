...
import UserPhotoProfileBIG from "../../shareds/ui/UserPhotoProfileBIG/UserPhotoProfileBIG";
import UserName from "../../shareds/ui/UserName/UserName";
import UserNetworkStatus from "../../shareds/ui/UserNetworkStatus/UserNetworkStatus";
import {IGroupChat, IRecentChat, IUser} from "../../shareds/types";
import './_CardActiveUser.scss';
...
export default function CardActiveUser() {
    ...
    const chatCheck = (chat: IGroupChat | IRecentChat | null): chat is IGroupChat => {
        if (chat) { return "nameChat" in chat } else { return false }
    };
        ...
        <article className={'user'}>
            ...
            <ul className={'user__info'}>
                <li className={'user__info-item'}>
                    ...
                    <span className={'user__info-item-val'}>{ currentUser?.job_title || 'Not Available' }</span>
                </li>
                <li className={'user__info-item'}>
                    <span className={'user__info-item-name'}>Email Address</span>
                    <a
                        href={`mailto:${chat?.users?.find( u => u._id !== my._id)?.email || '#'}`}
                        className={'user__info-item-val'}
                    >{ currentUser?.email || 'Not Available' }</a>
                </li>
                <li className={'user__info-item'}>
                    ...
                    <a
                        ...
                        className={'user__info-item-val'}
                    >{ currentUser?.phone || 'Not Available' }</a>
                </li>
                <li className={'user__info-item'}>
